...
from services.geo_service import geodesic_km, interpolate_towards
...
async def _move_towards_incident(
    ...
    distance_after_move = geodesic_km(next_lat, next_lng, incident["location_lat"], incident["location_lng"])
    updates = {"current_lat": round(next_lat, 6), "current_lng": round(next_lng, 6)}
    if distance_after_move <= 0.5:
...
async def _move_towards_hospital(
    ...
    distance_after_move = geodesic_km(next_lat, next_lng, hospital["lat"], hospital["lng"])
    updates = {"current_lat": round(next_lat, 6), "current_lng": round(next_lng, 6)}
    if distance_after_move <= 0.5:
