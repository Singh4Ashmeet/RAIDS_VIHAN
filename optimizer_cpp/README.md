...
- Check for the compiled binary at `optimizer_cpp/build/raid_optimizer`.
- On Windows CMake may produce `optimizer_cpp/build/raid_optimizer.exe`; deployment scripts can normalize or copy it to the expected path if needed.
...
The scaffold currently provides a standalone CLI named `raid_optimizer`. It reads a JSON file containing an `incident` object and an `ambulances` array, then writes a JSON assignment to stdout.
...
```bash
optimizer_cpp/build/raid_optimizer sample.json
```
