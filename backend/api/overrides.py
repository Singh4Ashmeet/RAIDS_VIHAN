...
from repositories.override_repo import OverrideRepository
...
async def _fetch_active_dispatch(dispatch_id: str) -> dict:
    dispatch = await DispatchRepository().get_by_id(dispatch_id)
    if dispatch is None or dispatch.get("status") != "active":
...
async def _fetch_available_ambulance(ambulance_id: str) -> dict:
    ...
    if ambulance is None or ambulance.get("status") != "available":
        _bad_request("Selected ambulance is not currently available")
    return ambulance


async def _fetch_open_hospital(hospital_id: str) -> dict:
    hospital = await HospitalRepository().get_by_id(hospital_id)
...
async def _move_hospital_patient_token(old_hospital_id: str, new_hospital_id: str, token: str) -> None:
    ...
    old_hospital = await hospital_repo.get_by_id(old_hospital_id)
    new_hospital = await hospital_repo.get_by_id(new_hospital_id)

    if old_hospital is not None:
        incoming = [item for item in list(old_hospital.get("incoming_patients", [])) if item != token]
        await hospital_repo.update(old_hospital_id, {"incoming_patients": incoming})

    if new_hospital is not None:
        incoming = list(new_hospital.get("incoming_patients", []))
        if token not in incoming:
            incoming.append(token)
        await hospital_repo.update(new_hospital_id, {"incoming_patients": incoming})
...
@router.post("/overrides/request", response_model=None)
async def request_override(
    ...
    await override_repo.create(override_record)
    ...
    response_payload = OverrideResponse(
        override_id=override_id,
        dispatch_id=original_dispatch["id"],
        status="approved",
        new_ambulance_id=payload.proposed_ambulance_id,
    ...
    return success(response_payload.model_dump(mode="json"), message="Override approved")
...
async def override_history(
    ...
    rows = await OverrideRepository().history(
        cutoff,
        city=city,
...
async def override_stats(
    ...
    return success({**stats, "most_common_override_reason": most_common})
...
async def available_units(
    ...
    ambulances = await AmbulanceRepository().get_all(city)
    hospitals = await HospitalRepository().get_all(city)
